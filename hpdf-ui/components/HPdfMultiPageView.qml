...
Item {
    ...
    */
    ...
    TableView {
        ...
        model:{
            return modelInUse && root.document ?root.document.pageCount : 0
        }
        ...
        property real pageHolderWidth:root.width//Math.max(root.width, ((rot90 ? document?.maxPageHeight : document?.maxPageWidth) ?? 0) * root.renderScale)
        columnWidthProvider: function() { return document ? pageHolderWidth + vscroll.width + 2 : 0 }
        rowHeightProvider:function() { return (rot90 ? firstPagePointSize.width : firstPagePointSize.height) * root.renderScale }
            //function(row) { return (rot90 ? document.pagePointSize(row).width : document.pagePointSize(row).height) * root.renderScale }

        delegate: Rectangle {

            id: pageHolder
            color: tableView.debug ? "beige" : "#f6f6f6"//"beige" : "transparent"
//            Text {
//                visible: tableView.debug
//                anchors { right: parent.right; verticalCenter: parent.verticalCenter }
//                rotation: -90;
//                text: pageHolder.width.toFixed(1) + "x" + pageHolder.height.toFixed(1) + "\n" +
